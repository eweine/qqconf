...
<<contingency_tables>>=
set.seed(8675309)
# Generate contingency tables for variables as defined above
...
<<chisq_qq, fig=TRUE, include=FALSE>>=
library(qqconf)
n <- 1000
bounds <- get_bounds_two_sided(alpha = .05, n = 1000)

# Plot 3: Q-Q plot of P-values from Chi-Squared Test
# for Independence in a 2 x 2 Table
...
# Add the Q-Q plot for scenario 1.
lines(c(1:n) / (n + 1), sort(pvals_scenario_1), col = "blue")
...
<<chisq_qq_log, fig=TRUE, include=FALSE>>=
# Plot 4: Q-Q plot, on the -log10 Scale, of P-values 
# from Chi-Squared Test for Independence in a 2 x 2 Table
n <- 1000
...
# Add the Q-Q plot for scenario 1.
lines(-log10(c(1:n) / (n + 1)), -log10(sort(pvals_scenario_1)), col = "blue")
...
<<base_qqplot, fig=TRUE, include=FALSE>>=
...
<<qqconf_qqplot, fig=TRUE, include=FALSE>>=
library(qqconf)
qq_conf_plot(
  obs = reg$residuals,
...
\begin{figure}
\begin{center}
\includegraphics{manuscript-qqconf_qqplot_ks}
\end{center}
...
We downloaded the p-values from a GWAS of Creutzfeld-Jakob disease in a sample of XXX cases and YYY controls. The GWAS found strong genetic associations on chromosomes A and B. Here, we remove those chromosomes from the results in order to display the p-values from tests that are not obviously from a non-uniform distribution. To more clearly allow for display of the individual p-values, we downsample the number of tests to 10,000 by extracting approximately equally spaced genetic markers.
<<load_data>>=
...
<<cjd_standard_qq, fig=TRUE, include=FALSE>>=
library(qqconf)
qq_conf_plot(
  obs = cjd_df[,3],
  distribution = qunif,
...
\begin{figure}
\begin{center}
\includegraphics[scale=1.5]{manuscript-cjd_standard_qq}
\end{center}
\caption{Standard QQ Plot for CJD GWAS p-values}
\end{figure}

Unfortunately, this plot results in deviations from the expected line that are difficult to see. To aid visualization, we also plot the deviation of the observed quantiles from the expected quantiles versus the expected quantiles (Figure 9). This can be done easily by setting the \code{difference} parameter to \code{TRUE} in \code{qq_conf_plot}.
<<cjd_diff_qq, fig=TRUE, include=FALSE>>=
qq_conf_plot(
  obs = cjd_df[,3],
  distribution = qunif,
  points_params = list(pch = 21, cex = 0.2),
...
\begin{figure}
\begin{center}
\includegraphics[scale=1.5]{manuscript-cjd_diff_qq}
\end{center}
\caption{QQ Plot With y-axis as Observed - Expected Quantile for CJD GWAS p-values}
\end{figure}

This plot is much easier to interpret, but note that tests in which there is an association will result in small p-values, which will be difficult to see when the plot axes are on the 0 to 1 scale. To see the small p-values we repeat the plots but with the axes on the -log10 scale (Figures 10 and 11). This can be done by setting the \code{log10} parameter of \code{qqconf} to \code{TRUE}

<<cjd_log_qq, fig=TRUE, include=FALSE>>=
qq_conf_plot(
  obs = cjd_df[,3],
  distribution = qunif,
  points_params = list(pch = 21, cex = 0.2),
...
\begin{figure}
\begin{center}
\includegraphics[scale=1.5]{manuscript-cjd_log_qq}
\end{center}
\caption{QQ Plot With y-axis as -$\textrm{log}_{10}$(Observed Quantile) for CJD GWAS p-values}
\end{figure}

<<cjd_log_diff_qq, fig=TRUE, include=FALSE>>=
qq_conf_plot(
  obs = cjd_df[,3],
  distribution = qunif,
  points_params = list(pch = 21, cex = 0.2),
...
\begin{figure}
\begin{center}
\includegraphics{manuscript-cjd_log_diff_qq}
\end{center}
