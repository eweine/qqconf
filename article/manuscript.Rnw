...
lines(-log10(bounds$x), -log10(bounds$x))

# Add the lower bound line for the level .05 
# simultaneous testing bands from equal local levels.
lines(-log10(bounds$x), -log10(bounds$lower_bound))

# Add the upper bound line for the level .05 
# simultaneous testing bands from equal local levels.
lines(-log10(bounds$x), -log10(bounds$upper_bound))

# Add the Q-Q plot for scenario 1, with axes on the -log10 scale.
lines(-log10(bounds$x), -log10(sort(pvals_scenario_1)), col = "blue")
